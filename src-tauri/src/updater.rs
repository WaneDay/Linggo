// Linggo 检查更新：启动后默认用 git ls-remote 探测 GitHub 远端 tag 检查新版本。
// 理由：api.github.com 匿名配额 60 次/时/IP，共享出口 IP 易超限弹 403；
// 改用 git 协议 ls-remote --tags（免认证免限流）打包最高 tag 探测，永不触网 api 限流。
// 仅检查不致动下载；有新版本 → 托盘系统气泡 + F5 主窗右下角圆形按钮；
...
/// 拉取 GitHub 远端 tag（轻量；免认证免限流：直接 ls-remote --tags 走 git 协议）。
/// 不碰 api.github.com —— 该端点匿名配额 60 次/时/IP，共享出口 IP 极易被限流弹 403。
fn latest_release_json() -> Result<serde_json::Value, String> {
    // 双通道：① api.github.com/releases/latest（主；本机实测通且免限额配额尚富余，能抓 tag）；
    // ②仅当 api 确切 403/超时 才回退 git ls-remote（绕过 git 协议但免认证；CREATE_NO_WINDOW 静默）。
    let api_url = format!("https://api.github.com/repos/{GITHUB_OWNER}/{GITHUB_REPO}/releases/latest");
    // 主通道用 Windows 自带的 System32\curl.exe（纯 std 子进程、CREATE_NO_WINDOW 静默）。
    // 无新依赖 reqwest（0.13.0 实测 200 拿到 v0.1.1）就能检查更新；只有 curl 非 200 才落 git 兜底。
    let curl_exe = if cfg!(target_arch = "x86_64") {
        r"C:\Windows\System32\curl.exe"
    } else {
        r"C:\Windows\SysWOW64\curl.exe"
    };
    let api_res = std::process::Command::new(curl_exe)
        .creation_flags(0x0800_0000) // CREATE_NO_WINDOW：curl 子进程静默后台，不弹控制台窗
    ...
    let out = std::process::Command::new("git")
        .creation_flags(0x0800_0000) // CREATE_NO_WINDOW：git 子进程静默后台，不弹控制台窗
        .env("GIT_TERMINAL_PROMPT", "0") // 禁止 git 终端交互提示（防止凭据询问触发新控制台）
        .env("GIT_ASKPASS", "") // 禁用 askpass 凭据助手 GUI
        .env("GIT_CREDENTIAL_HELPER", "") // 禁用凭据助手，避免弹 GUI 窗
...
fn show_balloon(latest: &str, url: &str) {
    ...
    let msg = format!("发现新版本 v{latest}。打开 Linggo 主窗，点击右下角圆形按钮查看发布页。\n{url}");
    let mut nid = NOTIFYICONDATAW::default();
    nid.cbSize = std::mem::size_of::<NOTIFYICONDATAW>() as u32;
    nid.hWnd = hwnd;
    nid.uID = 0; // 应用唯一托盘，tray-icon 内部计数器首个，uID 须为 0
    nid.uFlags = NIF_INFO;
